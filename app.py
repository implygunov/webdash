...
def login():
    ...
    if request.method == 'POST':
        ...
        password = request.form.get('password', '').strip()
        if login_val == ADMIN_LOGIN and verify_password(password, ADMIN_PASSWORD_HASH):
            session['admin'] = True
            session['user_id'] = 0
            session['login'] = 'admin'
            return redirect(url_for('admin'))
        conn = get_db()
        account = conn.execute("SELECT * FROM accounts WHERE login = ?", (login_val,)).fetchone()
        if not account or not verify_password(password, account['password_hash']):
            conn.close()
            flash('Неверный логин или пароль', 'error')
            return render_template('login.html', site_key=TURNSTILE_SITE_KEY)
        
        # Auto-rehash legacy password if needed
        if len(account['password_hash']) == 64:
            new_hash = hash_password(password)
            conn.execute("UPDATE accounts SET password_hash = ? WHERE id = ?", (new_hash, account['id']))
            conn.commit()
